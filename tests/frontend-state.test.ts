...
describe('frontend state helpers', () => {
  ...
  it('uses a unified story control dock with popover audio adjustments', async () => {
    ...
    expect(preview).toContain("{ value: 0.8, label: isEn ? 'Slow' : '慢速' }")
    expect(preview).toContain("{ value: 0.9, label: isEn ? 'Bedtime' : '睡前' }")
    expect(preview).toContain("{ value: 1, label: isEn ? 'Normal' : '原速' }")
    expect(preview).toContain("{ value: 1.2, label: isEn ? 'Fast' : '快速' }")
    ...
    expect(preview).toContain("aria-label={isEn ? 'Narration volume' : '人声音量'}")
  ...
  it('offers Chinese system voices alongside confirmation-gated online cloning', async () => {
    ...
    expect(library).toContain("title: isEn ? 'Save and clone this voice?' : '确认保存并在线复刻？'")
    expect(library).toContain("confirmLabel: isEn ? 'Confirm and start' : '确认并开始复刻'")
  ...
  it('keeps provider branding in settings while using neutral copy in the workflow', async () => {
    ...
    expect(library).toContain('Cloud built-in English voices')
    expect(library).toContain('云端内置中文音色')
    expect(library).not.toContain('MiniMax 云端内置音色')
    expect(panel).toContain('stepMessage(step.id, step.message, language)')
  ...
  it('places two bedtime recommendations first in each language', async () => {
    ...
    expect(library).toContain('voice-recommendation-badge">Recommended')
    expect(library).toContain('voice.bedtimeRecommendationReason')
    expect(composer).toContain('★ 推荐 · {voice.name}')
    expect(composer).toContain('Recommended · {voice.name}')
    ...
    expect(voices).toContain("name: 'Warm English Lady'")
    expect(voices).toContain("name: 'Gentle English Woman'")
    expect((voices.match(/bedtimeRecommendationRank:/g) || [])).toHaveLength(4)
  ...
  it('shows non-technical story details for each finished story', async () => {
    ...
    expect(preview).toContain("<dt>{isEn ? 'Background music' : '背景音乐'}</dt>")
